...
function buildMetric(
  ...
  const formatDelta = () => {
    if (Math.abs(delta) < 0.05 && format !== "number")
      return format === "days" ? "—" : "-0.0%";
    if (format === "currency") {
...
export function getPartnerLabel(partnerId: ComparePartnerId) {
  return (
    COMPARE_PARTNERS.find((partner) => partner.id === partnerId)?.label ??
    partnerId
  );
}
...
export function formatCompareDateRange(filters: CompareSideFilters) {
  ...
  if (filters.dateRange === "calendar-month") {
    const start = new Date(Number(filters.year), monthIndex, 1)
      .toISOString()
      .slice(0, 10);
    return `${start} → ${end}`;
  }
...
export function buildCompareSections(
  ...
  return [
    ...
    },
    {
      ...
      metrics: [
        buildMetric(
          "Average booking value (excl. fee)",
          leftAbvEx,
          rightAbvEx,
          "currency",
        ),
        buildMetric(
          "Average booking value (inc. fee)",
          leftAbvInc,
          rightAbvInc,
          "currency",
        ),
    ...
    },
    {
      ...
      metrics: [
        buildMetric(
          "CAL total payable",
          parseMetric(leftCal.totalPayable),
          parseMetric(rightCal.totalPayable),
          "currency",
        ),
        buildMetric(
          "IPT",
          parseMetric(leftCal.ipt),
          parseMetric(rightCal.ipt),
          "currency",
        ),
        buildMetric(
          "PISL commission",
          parseMetric(leftCal.pislComm),
          parseMetric(rightCal.pislComm),
          "currency",
        ),
        buildMetric(
          "Capacity net",
          parseMetric(leftCal.capacityNet),
          parseMetric(rightCal.capacityNet),
          "currency",
        ),
        buildMetric(
          "Premium inc. IPT",
          parseMetric(leftCal.premiumInc),
          parseMetric(rightCal.premiumInc),
          "currency",
        ),
        buildMetric(
          "GWP",
          parseMetric(leftCal.gwp),
          parseMetric(rightCal.gwp),
          "currency",
        ),
      ],
    },
    {
      ...
      metrics: [
        buildMetric(
          "Avg booking → stay (days)",
          leftBookingDays,
          rightBookingDays,
          "days",
        ),
        buildMetric(
          "Avg cancellation → stay (days)",
          leftCancelDays,
          rightCancelDays,
          "days",
        ),
