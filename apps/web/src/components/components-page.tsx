...
import {
  DesignSystemNavSidebar,
  referenceNavItems,
} from "@/components/design-system-nav-sidebar";
import { Input } from "@keystone/ui/primitives/input";
...
function ColorSwatch({
  ...
  token: (typeof colorPaletteTokens)[number];
}) {
  return (
...
function useDesignSystemPage(scrollRoot: HTMLElement | null) {
  ...
  const filteredCatalog = fullCatalog.filter((entry) => {
    ...
    const haystack = [
      ...
      ...getComponentCodeSnippets(entry).map(
        (snippet) => `${snippet.label} ${snippet.code}`,
      ),
    ]
      .join(" ")
      .toLowerCase();
  ...
  function scrollToSection(id: string) {
    ...
    const top =
      scrollRoot.scrollTop + (elementRect.top - rootRect.top) - SCROLL_OFFSET;
...
function DesignSystemPageContent({
  ...
  filteredCategories: ReturnType<
    typeof useDesignSystemPage
  >["filteredCategories"];
}) {
  return (
    <TooltipProvider>
      <div className="mx-auto max-w-4xl pb-16">
        <header className="space-y-4 border-b border-border pb-8">
          ...
          <h1 className="text-3xl font-semibold tracking-tight leading-9">
            Design system
          </h1>
          <p className="max-w-2xl text-[15px] leading-relaxed text-muted-foreground">
            Colours, typography, tokens, and a living library of every Keystone
            component — documented with live previews, props, usage examples,
            and source files.
          </p>
          ...
          <section id="color-palette" className="scroll-mt-24 space-y-8">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                <code className="text-xs">src/index.css</code>. Swatches reflect
                the active theme (light or dark).
              </p>
            </div>

            {colorGroups.map((group) => {
              const tokens = colorPaletteTokens.filter(
...
<div style={{ color: "var(--brand-pink)" }} />`}
          ...
          <section id="typography" className="scroll-mt-24 space-y-8">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Plus Jakarta Sans is the primary typeface across Keystone,
...
import { FIGURE_20PX_CLASS, FIGURE_24PX_CLASS } from "@keystone/ui/lib/figure-styles"`}
            />
          </section>
          ...
          <section id="design-tokens" className="scroll-mt-24 space-y-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Shared typography and layout constants used across widgets and
                charts.
              </p>
            </div>
            ...
            <CodeBlock
              code={`import { FIGURE_20PX_CLASS, FIGURE_24PX_CLASS } from "@keystone/ui/lib/figure-styles"
import { CHART_HEIGHT } from "@keystone/ui/lib/chart-styles"
...
export function DesignSystemView() {
  ...
  const { query, setQuery, activeId, scrollToSection, filteredCategories } =
    useDesignSystemPage(scrollRoot);
  ...
  return (
    <>
      <div className="min-h-0 min-w-0 overflow-hidden">
        <section
          ref={setScrollRoot}
          className="h-full overflow-y-auto px-20 py-12 xl:px-24 xl:py-14"
        >
