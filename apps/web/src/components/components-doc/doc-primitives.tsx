...
export function CodeBlock({
  ...
  if (embedded) {
    return (
      <pre className="overflow-x-auto bg-[#fafafa] p-4 text-[13px] leading-relaxed dark:bg-muted/20">
...
export function CodeExampleTabs({
  ...
  if (snippets.length === 1) {
    return <CodeBlock code={snippets[0].code} label={snippets[0].label} />;
  }

  return (
    <Tabs defaultValue={snippets[0].id} className="gap-0">
      <div className="overflow-hidden rounded-lg border border-border">
        <TabsList className="h-auto w-full justify-start rounded-none border-b border-border bg-muted/30 p-1">
          {snippets.map((snippet) => (
            <TabsTrigger
              key={snippet.id}
              value={snippet.id}
              className="flex-none px-3 data-[state=active]:bg-background"
            >
              {snippet.label}
            </TabsTrigger>
          ))}
        ...
        {snippets.map((snippet) => (
          <TabsContent key={snippet.id} value={snippet.id} className="mt-0">
            <CodeBlock code={snippet.code} embedded />
          </TabsContent>
        ))}
      </div>
    </Tabs>
...
export function PreviewFrame({
  ...
  return (
    ...
    >
      <div className={cn("grid gap-6 p-6", inset && "[&>*]:max-w-md")}>
        {children}
      </div>
    </div>
  );
...
export function ComponentDocBlock({ entry }: { entry: ComponentCatalogEntry }) {
  ...
  const codeSnippets = getComponentCodeSnippets(entry);

  return (
    <article
      id={entry.id}
      className="scroll-mt-24 space-y-6 border-b border-border pb-12 last:border-b-0"
    >
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_240px]">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <CodeExampleTabs snippets={codeSnippets} />
          </div>
