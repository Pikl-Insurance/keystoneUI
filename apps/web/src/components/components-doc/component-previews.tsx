...
import { InteractiveChartLegend } from "@keystone/ui/charts/interactive-chart-legend";
...
import { ExportSnapshotButton } from "@keystone/ui/widgets/export-snapshot-button";
...
import { Card, CardContent, CardHeader } from "@keystone/ui/primitives/card";
import { Input } from "@keystone/ui/primitives/input";
import { Label } from "@keystone/ui/primitives/label";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@keystone/ui/primitives/select";
...
import {
  ...
  TabsList,
  TabsTrigger,
} from "@keystone/ui/primitives/tabs";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from "@keystone/ui/primitives/tooltip";
import { BreakdownDataWidget } from "@keystone/ui/widgets/breakdown-data-widget";
import { DataSnapshotWidget } from "@keystone/ui/widgets/data-snapshot-widget";
import { DualDataListWidget } from "@keystone/ui/widgets/dual-data-list-widget";
import { GraphWidget } from "@keystone/ui/widgets/graph-widget";
import { HeadlineDataWidget } from "@keystone/ui/widgets/headline-data-widget";
import { WidgetHelpButton } from "@keystone/ui/widgets/widget-help-button";
import { BOOKING_ENGINE_PARTNERS } from "@/lib/booking-engine-data";
import { DEFAULT_FILTERS } from "@keystone/ui/lib/chart-data";
import {
...
function CompareFilterPanelPreview() {
  const [filters, setFilters] =
    useState<CompareSideFilters>(DEFAULT_COMPARE_SIDE);
...
function ChartLegendPreview() {
  const { hiddenKeys, toggleSeries } = useHiddenChartSeries([
    "made",
    "starting",
  ]);
...
export function ComponentPreview({ id }: { id: string }) {
  ...
  switch (id) {
        ...
        <PartnerVolumeWidget
          ...
          productSplit={{
            datasetA: {
              title: "With CAL",
              value: "1,104",
              clarification: "2.6% of bookings",
            },
            datasetB: {
              title: "With DDL",
              value: "12",
              clarification: "0.0% of bookings",
            },
          }}
        />
      );
    ...
    case "property-bookings-table":
      return (
        <PropertyBookingsTable bookings={MOCK_PROPERTY.bookings.slice(0, 5)} />
      );
    case "login-page":
      return (
        ...
        <Card className="max-w-sm">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Card content area for widgets and forms.
            </p>
          </CardContent>
        </Card>
      );
    ...
    case "ui-dropdown-menu":
      return (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <span>
            See App header user menu for full DropdownMenu composition.
          </span>
        </div>
      );
    default:
