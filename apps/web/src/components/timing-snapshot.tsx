...
const TIMING_ROW_DATA: Record<
  string,
  Array<{ avgLead: string; calAvgLead: string }>
> = {
...
function getTimingRows(filters: ActiveFilters) {
  ...
  const rowData =
    TIMING_ROW_DATA[key] ?? TIMING_ROW_DATA["all-partners:all-brands"];
      ...
      >
        <div className="@container min-w-0">
          <div
            className={cn(
              metricCardGridClass,
              "grid-cols-1 @4xl:grid-cols-[minmax(0,1fr)_minmax(0,240px)]",
            )}
          >
            <HeadlineDataWidget
