...
import { type ActiveFilters, getAbvProfile } from "@keystone/ui/lib/chart-data";
...
const ABV_ROW_DATA: Record<
  string,
  Array<{ abv: string; calAbv: string; abvIncFee: string; calPricePct: string }>
> = {
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                aria-label={
                  showBreakdown ? "Hide ABV breakdown" : "Show ABV breakdown"
                }
      ...
      >
        <div className="@container flex min-h-0 min-w-0 flex-1 flex-col">
          <div
            className={cn(
              metricCardGridClass,
              "grid-cols-1 @4xl:grid-cols-[minmax(0,220px)_minmax(0,1fr)]",
            )}
          >
            <HeadlineDataWidget
            ...
            <div
              className={cn(
                metricCardGridClass,
                "h-full min-h-0 grid-cols-1 @md:grid-cols-2",
              )}
            >
              <HeadlineDataWidget
