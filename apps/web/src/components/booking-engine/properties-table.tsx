...
function PostcodeCell({ postcode }: { postcode: string }) {
  ...
  return (
    <span className={cn("text-sm", isPlaceholder && "text-muted-foreground")}>
      {postcode}
    </span>
  );
}
        ...
        <TableBody>
          {properties.map((property) => (
            <TableRow key={property.code}>
              <TableCell className="px-5 py-4 text-sm tabular-nums">
                {property.code}
              </TableCell>
              ...
              <TableCell className="px-5 py-4 text-right">
                <Button
                  ...
                  onClick={() =>
                    property.propertyId && onViewProperty(property.propertyId)
                  }
                >
                  View
                </Button>
