...
function wave(offset: number, amp: number, period: number, phase = 0) {
  return (
    amp * (0.5 + 0.5 * Math.sin((2 * Math.PI * (offset + phase)) / period))
  );
}
...
const PROFILE: Record<
  string,
  { base: number; amp: number; trend: number; period: number }
> = {
...
export function buildDailyBookingsData(filters: ActiveFilters, days = 174) {
  ...
  return Array.from({ length: days }, (_, i) => ({
    ...
    made: Math.round(
      5000 * base +
        wave(i, 2500 * amp, period) -
        i * 10 * trend +
        seededNoise(seed, i) * 1200,
    ),
    starting: Math.round(
      3500 * base +
        wave(i, 4000 * amp, period / 2, 2) +
        seededNoise(seed + "s", i) * 800,
    ),
  }));
}
...
export function buildAbvPerDayData(filters: ActiveFilters, days = 174) {
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    partners.forEach((p, pi) => {
      row[p] = Math.round(
        1200 * base +
          wave(i, 1800 * amp, period, pi * 3) +
          seededNoise(seed + p, i + pi * 7) * 900,
      );
    });
    return row;
  });
...
export function buildLeadTimeData(filters: ActiveFilters, days = 174) {
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    series.forEach((s, si) => {
      row[s] = Math.round(
        90 * base +
          wave(i, 80 * amp, period * 1.5, si * 4) +
          seededNoise(seed + s, i + si * 11) * 40,
      );
    });
    return row;
  });
...
export function buildBookingsMadePerDayData(
  ...
  return Array.from({ length: days }, (_, i) => ({
    ...
    bookings: Math.round(
      4500 * base +
        wave(i, 3000 * amp, period) -
        i * 8 * trend +
        seededNoise(seed + "b", i) * 1500,
    ),
  }));
}
...
> = {
  "all-partners:all-brands": {
    gbpDays: "94.2 days",
    gbpCal: "CAL 118.7 days",
    eurDays: "108.5 days",
    eurCal: "CAL 134.1 days",
  },
  "partner-a:all-brands": {
    gbpDays: "88.4 days",
    gbpCal: "CAL 110.2 days",
    eurDays: "101.3 days",
    eurCal: "CAL 124.6 days",
  },
  "partner-b:all-brands": {
    gbpDays: "102.1 days",
    gbpCal: "CAL 128.4 days",
    eurDays: "116.7 days",
    eurCal: "CAL 144.3 days",
  },
  "partner-c:all-brands": {
    gbpDays: "91.6 days",
    gbpCal: "CAL 114.0 days",
    eurDays: "104.8 days",
    eurCal: "CAL 129.5 days",
  },
  "all-partners:brand-a": {
    gbpDays: "96.8 days",
    gbpCal: "CAL 121.4 days",
    eurDays: "111.2 days",
    eurCal: "CAL 137.8 days",
  },
  "all-partners:brand-b": {
    gbpDays: "86.3 days",
    gbpCal: "CAL 108.0 days",
    eurDays: "99.4 days",
    eurCal: "CAL 122.5 days",
  },
  "all-partners:brand-c": {
    gbpDays: "99.5 days",
    gbpCal: "CAL 124.8 days",
    eurDays: "114.6 days",
    eurCal: "CAL 141.2 days",
  },
};
