...
function fixSnapshotAlignment(root: HTMLElement) {
  root
    .querySelectorAll<HTMLElement>(
      "[data-snapshot-pill], [data-snapshot-filter-chip]",
    )
    .forEach((pill) => {
  ...
  root
    .querySelectorAll<HTMLElement>(".recharts-legend-item")
    .forEach((item) => {
  ...
  root
    .querySelectorAll<HTMLElement>(".recharts-tooltip-wrapper")
    .forEach((tooltip) => {
      tooltip.style.visibility = "hidden";
    });
}

export async function exportElementSnapshot(
  element: HTMLElement,
  filename: string,
) {
  ...
  const cardBg =
    getComputedStyle(document.documentElement)
      ...
      .trim() || "#ffffff";
  const excluded = element.querySelectorAll<HTMLElement>(
